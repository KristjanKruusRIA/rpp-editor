...
class RPPEditorGUI:
    ...
    def create_file_selection_widgets(self):
        ...
        self.file1_entry = ttk.Entry(file1_frame, textvariable=self.file1_var, width=50, state="readonly")
        self.file1_button = ttk.Button(file1_frame, text="Browse...", command=self.load_file1)
        
        self.file1_info_var = tk.StringVar()
        self.file1_info_label = ttk.Label(file1_frame, textvariable=self.file1_info_var, foreground="gray")
        
        # File 2 section
        file2_frame = ttk.LabelFrame(self.file_frame, text="File 2", padding=10)
        
        self.file2_var = tk.StringVar()
        self.file2_entry = ttk.Entry(file2_frame, textvariable=self.file2_var, width=50, state="readonly")
        self.file2_button = ttk.Button(file2_frame, text="Browse...", command=self.load_file2)
        
        self.file2_info_var = tk.StringVar()
        self.file2_info_label = ttk.Label(file2_frame, textvariable=self.file2_info_var, foreground="gray")
    ...
    def create_comparison_widgets(self):
        ...
        self.tracks1_tree = ttk.Treeview(tracks1_frame, columns=("volume", "pan", "effects"), 
        ...
        tracks1_scrollbar = ttk.Scrollbar(tracks1_frame, orient=tk.VERTICAL, command=self.tracks1_tree.yview)
        ...
        self.tracks2_tree = ttk.Treeview(tracks2_frame, columns=("volume", "pan", "effects"), 
                                        show="tree headings", height=15)
        ...
        tracks2_scrollbar = ttk.Scrollbar(tracks2_frame, orient=tk.VERTICAL, command=self.tracks2_tree.yview)
    ...
    def create_control_widgets(self):
        ...
        self.copy_left_button = ttk.Button(copy_frame, text="← Copy Selected to File 1", 
                                          command=self.copy_track_to_file1, state="disabled")
        self.copy_right_button = ttk.Button(copy_frame, text="Copy Selected to File 2 →", 
                                           command=self.copy_track_to_file2, state="disabled")
        ...
        copy_volume_check = ttk.Checkbutton(copy_options_frame, text="Volume", variable=self.copy_volume_var)
        copy_pan_check = ttk.Checkbutton(copy_options_frame, text="Pan", variable=self.copy_pan_var)
        copy_effects_check = ttk.Checkbutton(copy_options_frame, text="Effects", variable=self.copy_effects_var)
    ...
    def update_differences_display(self):
        ...
        if self.selected_track1 and self.selected_track2:
            ...
            self.diff_text.insert(tk.END, f"Comparing:\n")
    ...
    def copy_track_to_file1(self):
        ...
        try:
            self.parser1.copy_track_settings(
                self.selected_track2, self.selected_track1,
            ...
            self.update_tracks_display()
            self.update_differences_display()
            self.status_var.set(f"Copied settings from {self.selected_track2.name} to {self.selected_track1.name}")
    ...
    def copy_track_to_file2(self):
        ...
        try:
            self.parser2.copy_track_settings(
                self.selected_track1, self.selected_track2,
                copy_volume=self.copy_volume_var.get(),
                copy_pan=self.copy_pan_var.get(),
                copy_effects=self.copy_effects_var.get()
            )
            ...
            self.update_tracks_display()
            self.update_differences_display()
            self.status_var.set(f"Copied settings from {self.selected_track1.name} to {self.selected_track2.name}")
